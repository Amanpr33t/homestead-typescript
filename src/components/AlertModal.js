...
function AlertModal(props) {
    ...
    return (
        <Fragment>
            <div className="fixed z-50 top-16 pt-12 bg-transparent h-screen w-full flex justify-center " onClick={routeTo ? routeToPage : alertModalRemover}>
                <div className="relative w-11/12 sm:w-96 h-fit rounded-lg shadow bg-white" onClick={e => e.stopPropagation()}>
                    <button type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 " onClick={routeTo ? routeToPage : alertModalRemover}>
