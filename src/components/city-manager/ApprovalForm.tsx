...
const ApprovalForm: React.FC<PropsType> = (props) => {
    ...
    const formSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
        e.preventDefault()
        if ((sendBackToFieldAgent || sendBackToPropertyEvaluator) && (incompleteDetailsArray.length > 10 || incompleteDetailsArray.length === 0)) {
    ...
    return (
        <Fragment>
            ...
            <div className={`pt-24 pb-10 z-40 w-full fixed top-0 min-h-screen flex justify-center ${showApprovalForm ? '' : 'left-full'} ${alert.isAlertModal || showDetailsForm ? 'blur' : ''} `} onClick={hideApprovalForm}>
                <div className=" relative h-fit w-10/12 sm:w-9/12 md:w-7/12 bg-white rounded-md overflow-y-auto flex flex-col place-items-center px-2  md:px-0" onClick={e => e.stopPropagation()}>
                    ...
                    <form className="w-11/12 sm:w-8/12 flex flex-col rounded " onSubmit={formSubmit}>
            ...
            {showDetailsForm &&
                <div className={` z-40 w-full h-screen fixed top-0  bg-transparent flex justify-center pt-24 pb-10 ${alert.isAlertModal ? 'blur' : ''}`} onClick={(e) => {
                    ...
                    <div className={`relative h-fit max-h-full w-11/12 sm:w-9/12 md:w-7/12  rounded-md overflow-y-auto flex flex-col place-items-center px-2 md:px-0 pb-10 bg-white`} onClick={e => e.stopPropagation()}>

                        <div className="absolute -top-1.5 right-0.5 ">
                            {/*button to hide evaluation form*/}
                            <button type='button' className="text-3xl font-bold p-1 text-gray-600" onClick={hideApprovalForm}>X</button>
                        </div>

                        {/*heading */}
                        <p className="text-lg font-semibold w-full text-center mt-5">Add details of incomplete information</p>
