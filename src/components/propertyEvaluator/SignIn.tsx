...
*/
const PropertyEvaluatorSignIn: React.FC = () => {
    const navigate = useNavigate()
    const authToken: null | string = localStorage.getItem("homestead-property-evaluator-authToken")
    ...
    const signInFunction = async () => {
        ...
        setSpinner(true)
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/property-evaluator/signIn`, {
                method: 'POST',
            ...
            if (data.status === 'ok') {
                ...
                setAlert({
                    isAlertModal: true,
                    alertType: 'warning',
                    alertMessage: 'No evaluator agent with this email exists',
                    routeTo: null
                })
    ...
    const forgotPasswordRequest = async () => {
        ...
        setSpinner(true)
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/property-evaluator/forgotPassword`, {
                method: 'PATCH',
            ...
            if (data.status === 'ok') {
                ...
                setAlert({
                    isAlertModal: true,
                    alertType: 'warning',
                    alertMessage: 'No evaluator with this email exists',
                    routeTo: null
                })
            } else {
                throw new Error('Some error occured')
    ...
    const confirmOneTimePasswordFunction = async () => {
        setSpinner(true)
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/property-evaluator/confirmPasswordVerificationToken`, {
                method: 'POST',
                body: JSON.stringify({
    ...
    const updatePassword = async () => {
        ...
        setSpinner(true)
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/property-evaluator/updatePassword`, {
                method: 'PATCH',
                body: JSON.stringify({
                    email: emailForPasswordChange,
                    newPassword,
    ...
    const resetPasswordVerificationToken = async () => {
        setSpinner(true)
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/property-evaluator/resetPasswordVerificationToken`, {
                method: 'PATCH',
                body: JSON.stringify({ email: emailForPasswordChange }),
                headers: {
                    'Content-Type': 'application/json'
        ...
        <Fragment>
            ...
            <div className={`w-full h-screen pt-28 flex justify-center ${alert.isAlertModal ? 'blur-sm' : null}`} >
                <form className="w-full sm:w-96 p-4 mr-1.5 ml-1.5 flex flex-col bg-white rounded border-2 shadow-2xl h-fit" onSubmit={(e: ChangeEvent<HTMLFormElement>) => {
                    ...
                    {!isForgotPassword && <>
                        <label className="text-lg font-semibold mb-1" htmlFor="email-1">Property Evaluator email</label>
                        <input
                            type="email"
                            id="email-1"
                            name="email-1"
...
export default PropertyEvaluatorSignIn