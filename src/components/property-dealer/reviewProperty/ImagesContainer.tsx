...
const ImageContainer: React.FC<PropsType> = ({ isSold, imagesUrl, isLive }) => {
        ...
        <Fragment>
            <div className={`w-full ${showImagesInFullScreen ? 'h-screen bg-gray-900 fixed top-0 left-0 z-40 ' : 'bg-gray-100 pt-20 md:pt-20  flex justify-center h-fit w-full'}`}>
                ...
                <div className={`relative  ${showImagesInFullScreen ? 'flex items-center justify-center h-screen w-full' : 'w-full md:w-11/12 lg:w-9/12 h-fit bg-red-500'}`}>
                    ...
                    {isSold && !isLive && !showImagesInFullScreen && <p className="absolute top-5 left-5 px-1 py-0.5 bg-white rounded-lg font-semibold text-gray-800 text-sm flex flex-row"><GoDotFill className="mt-1 text-yellow-500" />Closed</p>}

                    {!isSold && isLive && !showImagesInFullScreen && <p className="absolute top-5 left-5 px-1 py-0.5 bg-white rounded-lg font-semibold text-gray-800 text-sm flex flex-row"><GoDotFill className="mt-1 text-red-600" />Live</p>}

                    {imagesUrl && imagesUrl.length > 1 && <button
                        className={`text-center absolute top-1/2 left-1 sm:left-5 transform -translate-y-1/2 bg-white bg-opacity-50 p-2 rounded-full font-extrabold ${showImagesInFullScreen && 'text-4xl'}`}
                    ...
                    {imagesUrl && imagesUrl.length > 1 && <button
                        className={`text-center absolute top-1/2 right-1 sm:right-5 transform -translate-y-1/2 bg-white bg-opacity-50 p-2 rounded-full font-extrabold ${showImagesInFullScreen && 'text-4xl'}`}
